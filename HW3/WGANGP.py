...
class WGANGP():
    ...
    def build_model(self):
        ...
        disc_gen_loss = tf.reduce_mean(disc_gen_output)
        disc_loss = disc_gen_loss - disc_real_loss
        
        alpha = tf.random.uniform([tf.shape(gen_img)[0],1,1,1],0,1)
        diff = gen_img - real_img
        interpolates = real_img + alpha*diff
        grad = tf.gradients(self.discriminator(interpolates,reuse=True),[interpolates])[0]
...
def train(epoch_num,batch_size=64,learning_rate=0.0002):
    ...
    graph_WGANGP = tf.Graph()
    with graph_WGANGP.as_default():
        model = WGANGP(lr = learning_rate)
    ...
    sess_train = tf.Session(graph=graph_WGANGP,config=config)
    ...
    for epoch in tqdm(range(epoch_num)):
        ...
        tqdm.write("Epoch %i, Gen Loss: %f, Disc Loss: %f, Disc Gen Loss: %f" %(epoch,gen_loss,disc_real_loss,disc_gen_loss))
        if (epoch+1)%50 == 0:
            model_path = saver.save(sess_train, './save/WGANGP/WGANGP', global_step=epoch_num*batch_num)
        if (epoch+1) % 20 == 0:
...
def test():
    print("Testing on WGAN-GP")
    graph_WGANGP = tf.Graph()
    with graph_WGANGP.as_default():
        model = WGANGP(lr = 0.0002)
        real_img,noise_input,_,_,_,_,_ = model.build_model()
        gen_infer_op = model.generator(noise_input,isTrain=False,reuse=True) 
        saver = tf.train.Saver(max_to_keep=3)
    config = tf.ConfigProto()   
    config.gpu_options.allow_growth = True
    sess_test = tf.Session(graph=graph_WGANGP,config=config)
    
    cur_dict = os.getcwd()
    print('Restoring model from:')
    print(cur_dict,'/save/\n')
    latest_checkpoint = tf.train.latest_checkpoint(cur_dict+'/save/WGANGP/old2')
